...
本地协议、客户端解析、调度与好友缓存测试目前为 **99/99 通过**。
...
- “设置 → 接口测试”进入时会自动加载服务器目录，并提供状态读取、全部目录检查以及喂食、洗澡、学习、打工、冒险的单次测试。目录检查是只读操作；带“真实测试”字样的按钮会立即消耗所选服务器道具或启动所选任务，并在操作后重新读取状态验证是否真正生效。

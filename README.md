...
| 食物与洗护库存 | 条件可用 | 按服务端食物列表读取饼干、虾仁；PC 会话未下发列表时显示“未下发”，不再用喂食次数冒充库存 |
| 自动补货与喂食 | 可用 | 金币购买后库存增加，喂食后体力增加 |
| 自动补货与洗澡 | 可用 | 优先购买并使用沐浴球，购买失败时回退已有香皂片 |
| 虾仁购买 | 暂不支持 | Android 与电脑 QQ 会话对同一购买命令返回不同商品，电脑端不会冒充成功 |
